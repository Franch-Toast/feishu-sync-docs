...
新创建的远端文档以**本地文件名**(去 `.md` 后的基名)作为云端标题:创建时通过 `docx/v1/documents` 显式携带 title 建空文档,再经 `docs_ai` overwrite 通道写入 Markdown 内容——正文中的 H1 不再决定云端文档名。同名额守卫因此只按**文件名**匹配:两个文件共享同一个一级标题不会再触发 `Remote folder already has a document named …`;只有远端已存在同名额文档时才会阻断并提示改名。若创建后内容写入失败,远端会留下一个空文档,下轮同步会自动认领这个同名空文档并补齐内容,不会重复建文档。存量文档(旧版以 H1 命名)不做自动改名,重新同步即可按新规则处理。
