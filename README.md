...
- **任务中心语义**:条目失败后单轮内自动重试 3 次(1s/2s/4s 退避),仍失败则进入「失败待处理」队列**等待人工处理**,不再跨轮无限重试;「重试」重新入队执行,「忽略」仅移除失败记录(条目保持 error 状态,文档仍正常参与后续同步评估,可随时重试);
...
3. 在「权限管理」中开通文档读写权限(创建文档需要「创建及编辑新版文档」`docx:document` 权限),并确保应用支持 `offline_access`(否则拿不到 refresh_token);
...
每轮同步按以下优先级配对两侧条目。第 0 级是「信封」机制：本地 `.md` 顶部 YAML 块里的 `feishu_token` 是文档**自描述的身份**，比任何启发式猜测都权威（详见 `docs/04-special-mechanisms.md`）：

1. **Tier-0 信封 token 配对**（最高权威）：文件 `feishu_token` ↔ 远端文档 token 精确命中；`IdentityResolver` 先按仲裁矩阵把信封与 `bindings.json` 对齐，声明了 token 的文件**绝不参与后续启发式配对**（铁律 R1）；
2. **remoteToken 精确配对**：`.feishu-sync/bindings.json` 里已绑定条目（`relativePath ↔ remoteToken`）直接走增量流程；
3. **路径配对**：远端相对路径（经文件名净化）与本地路径一致；
4. **标题配对**：本地文件基名（去 `.md`）与远端节点名归一化（NFC + trim）后相等且**唯一**；
5. **内容哈希配对**：两侧内容哈希一致的唯一候选自动绑定，多候选进冲突；
6. 仍未配对：远端有 → 导入本地；本地有 → 推送创建；双方都有但配不上 → 冲突工作台。

> 第 3–5 级（路径/标题/内容哈希）是「没带信封、也没绑定过」的裸文件才走的兜底层；一旦被 Tier-0/Tier-1 认领就不会再落入这些模糊层级。

文件名净化(`sanitizeLocalSegment`)统一处理 NFC 归一化、非法字符替换、HTML 实体残留、Windows 保留名与 255 字符截断,保证「远端文档名与本地文件名不一致(如存量文档以 H1 标题命名)」这类差异也能重新配对,而不是重复建文档。

### 远端命名对齐

新创建的远端文档以**本地文件名**(去 `.md` 后的基名)作为云端标题:创建时通过 `docx/v1/documents` 显式携带 title 建空文档,再经 `docs_ai` overwrite 通道写入 Markdown 内容——正文中的 H1 不再决定云端文档名。由于 `docs_ai` 的 Markdown 导入管线会用文档首个 H1 重写页面标题,引擎在**每次内容写入后校验云端标题**:若漂移为 H1,则通过 page block(`update_text_elements`)把标题钉回本地文件名,保证「云端文档名 = 本地文件名」恒成立。同名额守卫因此只按**文件名**匹配:两个文件共享同一个一级标题不会再触发 `Remote folder already has a document named …`;只有远端已存在同名额文档时才会阻断并提示改名。若创建后内容写入失败,远端会留下一个空文档,同轮自动重试会先失效远端树缓存,再认领这个同名空文档并补齐内容——即使创建请求已到达服务端但响应丢失,也不会重复建文档。存量文档(旧版以 H1 命名)不做自动改名,重新同步即可按新规则处理。

扫描阶段的**重复导入防护**:远端存在一个与某个本地文档内容完全一致但名字不同的未绑定文档(通常是旧版按 H1 命名时代留下的重复副本)时,引擎会跳过导入——不会为它创建本地文件;远端副本保留,由你在飞书云盘手动清理。
...
- 仍失败的条目进入任务中心「失败待处理」,**后续轮询不会自动重试**——需要人工「重试」或「忽略」:「重试」重新入队并出现在「进行中」;「忽略」**仅移除失败记录**(同条目的历史失败记录一并级联清除),条目保持 error 状态、文档仍正常参与后续同步评估,随时可再次重试;
- 已恢复为 pending/clean 的失败记录不再出现在失败队列;同一条目只保留最新一条失败记录;
- 支持逐条与 selection 批量重试 / 批量移除失败记录。
...
- **任务中心**:全局任务队列,按「进行中 / 失败待处理 / 已完成」分组。「进行中」区分「排队中」与「同步中」(正在执行的任务排在最前),每条耗时为该任务自身执行时长(不含排队等待,含自身重试);失败展示语义化错误分类(auth / permission / rate_limit / network…)+ 完整堆栈,支持批量重试 / 批量移除失败记录与「清空已完成」;
