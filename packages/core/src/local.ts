...
export class FilesystemProvider implements LocalProvider {
  async scan(root: SyncRoot): Promise<LocalFile[]> {
    ...
    await this.walk(resolve(root.localPath), resolve(root.localPath), files);
    return files;
  }
  ...
  private async walk(base: string, directory: string, output: LocalFile[]): Promise<void> {
    const entries = await readdir(directory, { withFileTypes: true });
    for (const entry of entries) {
      if (entry.name.startsWith(".") || entry.name === "node_modules") continue;
      const absolutePath = join(directory, entry.name);
      if (entry.isDirectory()) await this.walk(base, absolutePath, output);
        ...
        output.push({
          relativePath: relative(base, absolutePath).replaceAll("\\", "/"),
          absolutePath,
